...
import Animated, { cancelAnimation, Extrapolation, interpolate, runOnJS, useAnimatedReaction, useAnimatedStyle, useDerivedValue, useSharedValue, withDecay, withSpring, withTiming } from "react-native-reanimated";
...
export const GestureContainer = /*#__PURE__*/React.forwardRef(function GestureContainer({
  refreshHeight = 65,
  ...
  const stopScrollView = useCallback(() => {
    ...
    _ScrollTo(childScrollRef[curIndexValue.value], 0, scrollY + 0.1, false);
  ...
  const gestureHandlerHeader = useMemo(() => Gesture.Pan().activeOffsetY([-10, 10]).shouldCancelWhenOutside(false).enabled(scrollEnabled !== false).onBegin(() => {
  ...
  const gestureHandler = useMemo(() => Gesture.Pan().simultaneousWithExternalGesture(gestureHandlerHeader, ...childGestures).shouldCancelWhenOutside(false).enabled(scrollEnabled).activeOffsetX([-width, width]).activeOffsetY([-10, 10]).onBegin(() => {
    stopAllAnimation();
  }).onStart(() => {
  ...
  const renderRefreshControl = useCallback(() => {
    if (!onStartRefresh) return;
    return /*#__PURE__*/_jsx(RefreshControlContainer, {
      ...
      if (childScrollRef[curIndexValue.value]) {
        ...
        if (scrollY !== undefined) {
          if (mTrans > refreshHeight) {
            _ScrollTo(childScrollRef[curIndexValue.value], 0, mTrans - refreshHeight, false);
  ...
  const renderTabBarContainer = useCallback(children => {
    return /*#__PURE__*/_jsx(Animated.View, {
      ...
      children: /*#__PURE__*/_jsx(GestureDetector, {
        ...
        children: /*#__PURE__*/_jsxs(Animated.View, {
          ...
          children: [renderScrollHeader && /*#__PURE__*/_jsx(View, {
            onLayout: headerOnLayout,
            children: renderScrollHeader()
          }), navigationState?.routes.length === 0 && emptyBodyComponent ? /*#__PURE__*/_jsx(View, {
            style: {
  ...
  const renderSceneHeader = useCallback((children, props) => {
